...
<title>Mirvat FIQ · TritoX Quality Check</title>
  ...
  <div class="brand-header">
    <div class="brand-left">
      ...
      <div>
        ...
        <div class="brand-sub">Mirvat FIQ · Quality Check System · Auto &amp; Bundle</div>
      </div>
    </div>
...
function analyzeQuote(text,filename){
  ...
  if(isHomeBundle){
    // Mirvat Home rule: Standard coverage option and $5,000 All Perils deductible.
    // Replacement Cost selections are not QC requirements for this agency.
    homeData.coverageStandardOk=/Coverage\s+option\s+Standard/i.test(t);
    if(!homeData.coverageStandardOk) errors.push('Home: Coverage option must be Standard');
  ...
  return '<div class="row-details-grid" style="margin-top:0">'
    ...
    +(isRenters
      ...
      :(h.coverageStandardOk
        ?'<div class="check-item"><span class="ci-icon ci-ok">✓</span><span class="ci-label">Coverage Option</span><span class="ci-val ok">Standard</span></div>'
        :'<div class="check-item"><span class="ci-icon ci-fail">✗</span><span class="ci-label">Coverage Option</span><span class="ci-val bad">Must be Standard</span></div>'))
    +(h.allPerilsOk
